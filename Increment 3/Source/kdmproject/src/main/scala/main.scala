...
object main {
  def main(args : Array[String]): Unit =
  {
    ...
    val stopWordsInput = sc.textFile("E:\\UMKC\\Sum_May\\KDM\\week 7\\Tutorial 7A-SourceCode\\Tutorial 7A-SourceCode\\SparkOpenIE_WordNET_LDA\\data\\stopwords.txt")
    val stopwords = stopWordsInput.flatMap(x=>x.split(",")).map(_.trim)
    val broadcastStopWords = sc.broadcast(stopwords.collect.toSet)
   val input = sc.textFile("E:\\UMKC\\Sum_May\\KDM\\week1\\bbcsport\\cricket\\Set1\\")
   ...
   val NLPdataforclasses =ner.map(c =>
    ...
    {
      val iarray = i.split(" ")
     if(!(iarray(1).filter(!broadcastStopWords.value.contains(_)).isEmpty)) {
       iarray.mkString(",")
     }
    })

    nlpdataforindividuals.collect().foreach(Individuals_outfile.println)
